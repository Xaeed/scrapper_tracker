...
export default function ScraperConfigPage() {
  ...
  const [jobTypes, setJobTypes] = useState<string[]>(['C'])
  const [workplaceTypes, setWorkplaceTypes] = useState<string[]>(['2'])
  ...
  useEffect(() => {
    fetch('/api/scraper-config')
      ...
      .then((data: Config) => {
        ...
        setJobTypes(data.jobTypes ?? ['C'])
        setWorkplaceTypes(data.workplaceTypes ?? ['2'])
